...
const PaymentForm = ({id}) => {
    ...
    const handlePayment = async () => {
        ...
        const {data:statusUpdate}  =  await axios.patch("http://localhost:8500/update-status", {
                id,
                    payment_status: "paid",
                  });
    
         const {data:history} =  await axios.post(`http://localhost:8500/add-payment-history`,{
                camp_name:camp?.camp_name,
                fee:camp?.camp_fee,
                payment_status:camp?.payment_status,
                email:user?.email,
